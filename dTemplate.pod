...
From a scalar variable:
    ...
    $template = dTemplate->new( text => $filename );
...
When an encoder is called, then the first parameter is the encodable text,
and the second parameter is the optional encoder parameter, which can be
provided in the placeholder, e.g:
